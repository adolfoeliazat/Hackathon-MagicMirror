      ...
      var onClick = function (event) {
        ...
        if (!elementWasOpen && !element.hasClass('disabled') && !element.prop('disabled')) {
          ...
          if (controller.small()) {
            ...
            var textElement = $position.position(element.children());
            var leftOfElement = offset.left - parentOffset.left;
            var leftOfText = textElement.left;
            var left = Math.round(leftOfText - ((dropdownWidth - (textElement.width)) / 2));
            var rightThreshold = parentOffset.width - dropdownWidth - 8;
            if (left > rightThreshold) {
              left = Math.round(leftOfElement - dropdownWidth + $position.position(element).width);
              dropdown.removeClass('left').addClass('right');
            } else if (left < 0) {
              left = Math.round(leftOfElement);
            }
            css.left = left + 'px';
            css.position = null;
            css['max-width'] = null;
